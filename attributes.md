...
Attributes可以为你日历上的某些日期加上一些标记，使你的日历更加生动。

<guide-attributes-intro />

Attributes是一个对象数组（每个对象就是一个单独的属性）。

一个属性可以使用在某一天、某个日期范围，甚至是复杂的日期模式（比如：每隔一个星期五、每个月的15号、每隔一个月的最后一个星期五）
...
data() {
  return {
    ...
    attributes: [
      // 这是一个单独的属性
      {
        // 可选的key，可以用来在后面获取这个属性，一般是从你的数据对象中获取（也就是给你的属性加一个唯一标识符，方便你后面找到它）
        key: Any,
        // 属性的类型
        highlight: true,  // 高亮类型：支持Boolean, String, Object
        dot: true,        // 点类型：支持Boolean, String, Object
        bar: true,        // 条类型：支持Boolean, String, Object
